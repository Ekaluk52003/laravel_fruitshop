...
<nav x-data="{ open: false }" class="text-white bg-green-800 border-b border-green-700 dark:bg-green-900 dark:border-green-800">
    ...
    <div class="px-4 mx-auto max-w-7xl sm:px-6 lg:px-8">
        <div class="flex justify-between h-20">
            <div class="flex">
                ...
                <div class="flex relative items-center shrink-0">
                    <a href="{{ route('home') }}?locale={{ app()->getLocale() }}" class="flex items-center">
                        ...
                        <div class="hidden sm:flex sm:items-center">
                            <img class="w-16 h-16" src="{{ asset('images/branding/pinveganlogo.webp') }}" alt="{{ __('frontend.fruit_shop') }}">
                        ...
                        <div class="flex items-center sm:hidden">
                            <div class="shadow-sm">
                                <img src="{{ asset('images/branding/pinveganlogo.webp') }}" alt="{{ __('frontend.fruit_shop') }}" class="w-14 h-14 rounded-full">
                            </div>
                ...
                <!-- Navigation Links -->
                <div class="hidden gap-6 items-center sm:ms-10 sm:flex">
                    <x-ui.nav-link :href="route('home') . '?locale=' . app()->getLocale()" :active="request()->routeIs('home')" class="flex items-center mx-2 h-20">
                        {{ __('frontend.home') }}
                    </x-ui.nav-link>
                    
                    <x-ui.nav-link :href="route('fruits.index') . '?locale=' . app()->getLocale()" :active="request()->routeIs('fruits.*') && !request()->has('category_id')" class="flex items-center mx-2 h-20">
                    ...
                    @foreach($topLevelCategories as $topCategory)
                        ...
                        <div x-data="{ open: false }" @click.outside="open = false" @close.stop="open = false" class="flex relative items-center mx-2 h-20">
    ...
    class="absolute left-0 z-50 w-48 rounded-md shadow-lg origin-top-left"
    style="display: none; top: 100%;">
                        ...
                        <x-ui.nav-link :href="route('fruits.index') . '?locale=' . app()->getLocale() . '&category_id=' . $topCategory->id . '&sort=name&direction=asc'" :active="request()->routeIs('fruits.*') && request()->has('category_id') && request()->category_id == $topCategory->id" class="flex items-center mx-2 h-20">
                            {{ $topCategory->name }}
                        </x-ui.nav-link>
                        @endif
                    @endforeach                    
                    
                    <x-ui.nav-link :href="route('quote.index') . '?locale=' . app()->getLocale()" :active="request()->routeIs('quote.*')" class="flex items-center mx-2 h-20">
            ...
            <div class="hidden sm:flex sm:items-center">
                            ...
                            <form action="{{ route('fruits.index') }}" method="GET">
                                ...
                                <div class="relative">
                                    <input type="text" name="search" placeholder="{{ __('frontend.search_fruits') }}" value="{{ request('search') }}" 
                                           class="py-2 pr-10 pl-3 w-full text-sm text-gray-900 bg-white rounded-md border border-gray-300 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                           autofocus>
                                    <button type="submit" class="flex absolute inset-y-0 right-0 items-center px-3 text-gray-600 hover:text-green-700">
                ...
                @auth
                    <x-ui.dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button class="inline-flex items-center px-3 py-2 text-sm font-medium leading-4 text-white rounded-md border border-transparent transition duration-150 ease-in-out hover:text-yellow-400 focus:outline-none focus:bg-green-700 focus:text-yellow-400">
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="px-4 py-3" x-data="{ mobileSearchOpen: false }">
            <button @click="mobileSearchOpen = true" class="flex items-center px-4 py-2 w-full text-base font-medium text-white transition duration-150 ease-in-out hover:bg-green-700 focus:outline-none focus:bg-green-700">
